                ...
                for(i ; i < limit ; i ++ ) {
                    ...
                    if(result1 != undefined) {
                        ...
                        if(result2 != undefined) {
                            arr_url.push({id:arr_idUrl[i] ,oldUrl: result1.url, newUrl: result2.url, totalClick: result2.totalClick,timeCreate: result1.timeCreate });
                        } 
                    } 
                }
            }
    ...
    try {
        ...
        if(hostname == domain){
            if(path1.length > 0 && regex.test(path1)){
                ...
                //     customer.state = "ok";
                //     let last_page = lastPaste(totalRecord, 5);
...
let addLink1 = async (oldUrl, newUrl, user) => { 
    ...
    // let shortUrl = seedUrl.createShortUrl();
    try{
        let ob_shortUrl = await Shorten.save({url: newUrl}); console.log("ob_shortUrl:", ob_shortUrl)
...
//             if(path1.length > 0 && regex.test(path1)){
//                 let valid = await Url.checkUpdate(newUrl, req.session.user);
//                 if(valid){
//                     //console.log("ok 3");
//                     customer.state = "fail";
//                 } else {
//                     customer.state = "ok";
//                     let result = await Url.update(req.body.id, req.body); 
//                 }
//             }else{
//                 customer.state = "fail";
//             }
...
//     } catch (e) {
//         console.log("Error userEditLink");
//     }
// }
